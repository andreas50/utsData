...
#' 
#' This function imports the lynx data from a local CSV file and returns it as as \code{\link{uts_vector}} object.
#' 
#' @return A \code{\link[utsMultivariate:uts_vector]{uts_vector}} object. An observation for date, let's say, 1840-05-31 represents the total number of lynx catches for outfit 1839, which ran from 1 June 1839 to 31 May 1840.
#' @seealso The already imported data can be accessed via \code{\link{lynx_1942}}.
#' @seealso The \code{\link[datasets:lynx]{lynx}} dataset in base \R is similar, except that it (i) contains data for only a single geographic region (instead of all eleven regions), (ii) has an obvious typo (compared to its quoted source).
#' 
...
#' The annual number of Canadian lynx (\emph{Lynx canadensis}) catches by the Hudson's Bay Company. Each recorded year (called \emph{Outfit}) counts collected furs from 1 June of that year until 31 May of the following year.
#'
#' @format A \code{\link[utsMultivariate:uts_vector]{uts_vector}} object. An observation for date, let's say, 1840-05-31 represents the total number of lynx catches for outfit 1839, which ran from 1 June 1839 to 31 May 1840.
#'
...
#'
#' @seealso \code{\link{import_lynx_1942}}
#' @seealso The \code{\link[datasets:lynx]{lynx}} dataset in base \R is similar, except that it (i) contains data for only a single geographic region (instead of all eleven regions), (ii) has an obvious typo (compared to its quoted source).
#' 
#' @name lynx_1942
#' @keywords datasets
